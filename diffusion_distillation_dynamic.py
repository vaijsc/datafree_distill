...
def main(args):
    ...
    image_dir = f"{experiment_dir}/image"
    x0_dir = f"{experiment_dir}/x0"
    loss_dir = f"{experiment_dir}/loss"
    ...
    if args.resume_ckpt is None:
        ...
        parts = args.resume_ckpt.split('_')
    ...
    if rank == 0:
        if args.resume_ckpt is None:
            ...
            os.makedirs(f'{experiment_dir}/{args.resume_ckpt}', exist_ok=True)
            logger = create_logger(f'{experiment_dir}/{args.resume_ckpt}')
            logger.info(f"Resume writing at directory at {experiment_dir}/{args.resume_ckpt}, starting at state {start_state}, iter {start_iter}")
    else:
        pass
    ...
    if args.resume_ckpt is not None:
        resume_path = os.path.join(experiment_dir, f"checkpoints/{args.resume_ckpt}")
    ...
    n_iter = args.based_iter // dist.get_world_size()
    n_iter_to_update_schedule = n_iter // 10

    for idx in range(start_state, args.nfe-1):
        if rank == 0:
            logger.info(f"========== Update time {idx+1} ===========")
        loss_file_path = f"{loss_dir}/loss_{args.nfe-idx-1}.txt"
        if rank == 0:
            with open(loss_file_path, 'w') as loss_file:
                loss_file.write("")
        # opt = torch.optim.RMSprop(model.parameters(), lr=1e-6, alpha=0.9, eps=1e-8, weight_decay=0.0)
        learning_rate = 1e-4
        opt = torch.optim.Adam(model.parameters(), lr=learning_rate)
        for train_steps in tqdm(range(start_iter, n_iter)):
            ...
            # student engage
            # t = torch.randint(diffusion.num_timesteps-idx-1, diffusion.num_timesteps, (noise.shape[0],), device=device) # from T -> (T-assistant_delta_t-skip)
            ...
            z0_next_t_student = diffusion.q_sample(out["pred_xstart"], T-idxs-1, noise=eps)
            ...
            # z0t_next_t_student = diffusion.q_sample(out["pred_xstart"], T-idxs-1, noise=eps)
            ...
            # # loss = torch.mean(huber_loss(z0t_next_t_student, z_next_t_teacher)) 
            # loss = mae_loss(z0_next_t_student, z_next_t_teacher)
            # loss = mse_loss(z0_next_t_student, z_next_t_teacher)
            loss = huber_loss(z0_next_t_student, z_next_t_teacher, delta = 0.001)
            
            if train_steps % 50 == 0 and rank == 0:
                with open(loss_file_path, 'a') as loss_file:
                    loss_file.write(f"{mae_loss(z0_next_t_student, z_next_t_teacher).item()}\n")
            loss.backward()
            opt.step()

            if train_steps % n_iter_to_update_schedule == 0 and train_steps < n_iter_to_update_schedule * 8:
                learning_rate /= 2
                for param_group in opt.param_groups:
                    param_group['lr'] = learning_rate
                if rank == 0:
                    print(f'Learning rate of optimizer in step {train_steps + 1} is {learning_rate}')
            ...
            if train_steps % 2000 == 0:
                if rank == 0:
                    with torch.no_grad():
                        ...
                        x0_list = [vae.decode(x0 / args.scale_factor).sample for x0 in x0_list]
                        x0_list = torch.cat(x0_list, dim=0)
                        torchvision.utils.save_image(x0_list, os.path.join(x0_dir, f'x0_s{idx:02d}_iter{train_steps:07d}.jpg'), normalize=True, nrow=4)
                        torchvision.utils.save_image(fake_image, os.path.join(image_dir, f'image_s{idx:02d}_iter{train_steps:07d}.jpg'), normalize=True)
                        model.train()
